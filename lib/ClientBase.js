		...
		// Messages with "tmi.twitch.tv" as a prefix
			...
			switch(message.command) {
				...
				case 'USERSTATE':
					...
					if(nonce) {
						//Search if a message with tthat nonce is pending send
						...
						//This flag is here to differentiate messages sent from this client from others
						//This allows to know which message emotes should be parsed statically or not
						userstate.sentLocally = true;

						//If message was answering to another message, inject its ID to the tags
						...
						// Message is a regular chat message
							...
							this.log[messagesLogLevel](`[${channel}] <${this.getUsername()}>: ${message}`);
							this.emits([ 'chat', 'message' ], [
								[ channel, userstate, message, true ]
							]);
						}
